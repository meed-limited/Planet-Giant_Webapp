...
const DisplayNFT = ({ userNFTs, action, chainId, isSelection, setSelectedNFT, setVisibility }) => {
  ...
  const getSerieAttributes = (nft) => {
    let metaJSON = nft.metadata;
    if (metaJSON !== null) {
      for (let i = 0; i < metaJSON.attributes.length; i++) {
        if (metaJSON.attributes[i].trait_type && metaJSON.attributes[i].trait_type === "Series") {
          ...
          {userNFTs?.result &&
            userNFTs.result.map((nft, index) => {
              ...
              return (
                <Card
                  hoverable
                  onClick={isSelection ? () => handleSelect(nft) : undefined}
                  actions={
                    action && chainId
                      ? [
                          ...
                          <Tooltip title="Transfer NFT">
                            <SendOutlined onClick={() => action(nft)} />
                          </Tooltip>,
